...
def MENU():
	...
	#addDir3("Episodios Recentes", 'http://www.animesorion.tv/page/1', 8,'http://i.imgur.com/N3COLLn.png','http://i.imgur.com/a3TSg6N.jpg','Recents')
...
def LastEp(url):
	...
	s = int(next_pageID)+1
	next_page = "http://www.animesorion.tv/page/%d"%s
	match = re.compile('(?s)(?<=<div class=\"cAlign\">).*?(?=<\/ul>)',re.DOTALL).findall(r.content)
	image = re.compile('<img src=\"(.+?)\"',re.DOTALL).findall(str(match))
	...
	for _image in image:
		__image = _image
	for url, name in __match:
		addDir3(name, url, 9, __image, __image, '')
